 ...
 */
public abstract class APITask extends RemoteAsyncTask<APIResponse> {
     ...
     */
    public APITask(final boolean autoInterface) {
...
//			dialog = showCancelableDialog(getActivity(), getTask(), getActivity().getString(R.string.please_wait_));
    ...
    @Override
    protected final void onSuccess(final APIResponse result) {
        dismiss();
        if (result != null) {
            try {
                onComplete(result);
                ...
                e.printStackTrace();
//				onError(getActivity().getString(R.string.Unkown_error_has_occurred_please_check_your_Internet_connection_));
            }
        } else {
//			getActivity().showToast(R.string.Your_session_has_expired_please_sign_in_again);
        }
    }
